...
static const char *frag_particles =
    ...
    "    vec3 col = vec3(0.005,0.005,0.015); float t = u_time;\n"
    "    for(int i=0;i<250;i++){\n"
    "        float fi=float(i);\n"
    "        float z = fract(hash(vec2(fi*0.73,fi*1.17)) + t*0.25*(0.3+hash(vec2(fi*3.1,0.0))*0.7));\n"
    "        float depth = 1.0/((1.0-z)*3.0+0.15);\n"
    "        vec2 p = vec2(hash(vec2(fi,1.0))-0.5, hash(vec2(1.0,fi))-0.5) * depth * 0.6;\n"
    "        vec2 diff = uv - p;\n"
    "        float d = length(diff);\n"
    "        float bval = spec(mod(fi*2.0,64.0)/64.0);\n"
    "        float sz = min((0.004+bval*0.006)*depth*(1.0+u_beat*0.3), 0.06);\n"
    "        float glow = sz/(d*d+sz*0.08);\n"
    "        float star_hue = hash(vec2(fi*2.7,fi*0.43));\n"
    "        float hue = star_hue<0.3 ? 0.6+star_hue*0.3 : (star_hue<0.6 ? 0.05+star_hue*0.1 : star_hue*0.15);\n"
    "        float sat = 0.2+star_hue*0.5;\n"
    "        col += hsv2rgb(vec3(mod(hue+t*0.01,1.0),sat,1.0)) * glow * 0.08;\n"
    "    }\n"
    "    col += vec3(0.6,0.7,0.9)*0.01/(length(uv)+0.08)*u_energy;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";
...
static const char *frag_fireballs =
    ...
    "    for(int i=0;i<35;i++){float fi=float(i);\n"
    "        float h1=hash(vec2(fi*1.23,fi*0.77));\n"
    "        float h2=hash(vec2(fi*2.71,fi*1.43));\n"
    "        float h3=hash(vec2(fi*0.67,fi*3.11));\n"
    "        float h4=hash(vec2(fi*3.91,fi*0.53));\n"
    "        float spd_x=1.2+h1*1.8;\n"
    "        float spd_y=1.0+h2*2.0;\n"
    "        float bx=abs(mod(h3*10.0+t*spd_x*0.15,2.0)-1.0);\n"
    "        float by=abs(mod(h4*10.0+t*spd_y*0.18,2.0)-1.0);\n"
    "        bx=0.05+bx*0.9;\n"
    "        by=0.05+by*0.9;\n"
    "        vec2 diff=vec2((uv.x-bx)*aspect, uv.y-by);\n"
    "        float d=length(diff);\n"
    "        float bval=spec(mod(fi*2.0,64.0)/64.0);\n"
    "        float r=0.035+u_bass*0.008+u_beat*0.01;\n"
    "        if(d<r*3.0){\n"
    "            float core=exp(-d*d/(r*r*0.8))*(0.6+bval*0.5+u_energy*0.3);\n"
    "            float outer=exp(-d*d/(r*r*4.0))*0.4;\n"
    "            float flicker=0.85+0.15*sin(fi*7.0+t*12.0+bval*5.0);\n"
    "            float flame_v=core+outer;\n"
    "            flame_v*=flicker;\n"
    "            vec3 fire;\n"
    "            if(flame_v>0.7) fire=mix(vec3(1.0,0.6,0.1),vec3(1.0,1.0,0.8),(flame_v-0.7)/0.3);\n"
    "            else if(flame_v>0.3) fire=mix(vec3(0.8,0.15,0.02),vec3(1.0,0.6,0.1),(flame_v-0.3)/0.4);\n"
    "            else fire=vec3(0.8,0.15,0.02)*flame_v/0.3;\n"
    "            fire+=vec3(0.2,0.08,0.0)*u_beat*0.5;\n"
    "            col+=fire;\n"
    "        }\n"
    "    }\n"
    "    col+=vec3(0.15,0.05,0.02)*u_beat*0.15;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";
...
static const char *frag_dna =
    ...
    "        float orbit_a=t*0.25+fs*1.2566;\n"
    "        float orbit_r=0.5+sin(t*0.15+fs*2.0)*0.3;\n"
    "        float cx=cos(orbit_a)*orbit_r;\n"
    "        float cy=sin(orbit_a)*orbit_r;\n"
    "        float rot=t*0.6+fs*0.7;\n"
    ...
    "                float sp=spec(mod(fs*8.0+floor(scroll/1.5),64.0)/64.0);\n"
    "                col+=hsv2rgb(vec3(mod(hue+0.3,1.0),0.5,1.0))*rung*fade*(0.5+sp);\n"
    "            }\n"
    "            float elec=noise(vec2(rl.y*40.0+fs*10.0,t*8.0))*0.015/(dx+0.01)*sz*fade;\n"
...
static const char *frag_matrixrain =
    ...
    "    float digit_val=hash(vec2(col_id,row_id+floor(t*2.0*(0.5+col_hash))));\n"
...
static const char *frag_rainbowbubbles =
    ...
    "        float vx=sin(fi*3.7)*0.3+cos(fi*1.3+0.5)*0.2;\n"
    "        float vy=cos(fi*2.9)*0.3+sin(fi*0.7+1.0)*0.2;\n"
    "        float spd=0.3+bh3*0.4+u_energy*0.3;\n"
    "        float bx=bh2*2.0-1.0+vx*t*spd;\n"
    "        float by=bh3*2.0-1.0+vy*t*spd;\n"
    "        float bnd_x=ar-r;\n"
    "        float bnd_y=1.0-r;\n"
    "        bx=bnd_x-abs(mod(bx+bnd_x,bnd_x*4.0)-bnd_x*2.0);\n"
    "        by=bnd_y-abs(mod(by+bnd_y,bnd_y*4.0)-bnd_y*2.0);\n"
    "        bx+=sin(t*0.8+fi*2.7)*0.08*(1.0+u_bass*0.5);\n"
    ...
    "            float hue=fract(film_thick*0.3+fi*0.07+t*0.1+u_bass*0.15+u_energy*0.1);\n"
    "            float sat=0.6+0.3*sin(film_thick*4.0+u_mid*2.0);\n"
    ...
    "            bubble+=hsv2rgb(vec3(mod(hue+0.5,1.0),0.8,1.0))*u_beat*0.2*rim;\n"
