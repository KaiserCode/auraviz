...
static const char *frag_neonconstellation =
    "void main() {\n"
    "    vec2 uv=(gl_FragCoord.xy/u_resolution-0.5)*2.0*vec2(u_resolution.x/u_resolution.y,1.0);\n"
    "    vec2 uv01=gl_FragCoord.xy/u_resolution; float t=u_time;\n"
    "    float dist=length(uv)+0.001; float ang=atan(uv.y,uv.x);\n"
    "    float ks=6.0; float ka=mod(ang,6.283/ks); if(ka>3.14159/ks) ka=6.283/ks-ka;\n"
    "    vec2 kuv=vec2(cos(ka),sin(ka))*dist;\n"
    "    float fire_n=noise(vec2(kuv.x*5.0,(kuv.y+1.0)*4.0-t*2.0))*0.5\n"
    "        +noise(vec2(kuv.x*10.0,(kuv.y+1.0)*8.0-t*3.5))*0.3;\n"
    "    fire_n*=(0.35+u_energy*0.5+u_beat*0.2);\n"
    "    float fv=clamp(fire_n,0.0,1.0);\n"
    "    vec3 col;\n"
    "    if(fv<0.25) col=vec3(fv*3.0+0.12,fv*0.3+0.04,0.02);\n"
    "    else if(fv<0.5){float f=(fv-0.25)*4.0;col=vec3(0.7+f*0.3,f*0.5+0.04,0.02);}\n"
    "    else if(fv<0.75){float f=(fv-0.5)*4.0;col=vec3(1.0,0.5+f*0.4,f*0.2);}\n"
    "    else{float f=(fv-0.75)*4.0;col=vec3(1.0,0.9+f*0.1,0.2+f*0.6);}\n"
    "    col*=0.35;\n"
    "    for(int s=0;s<50;s++){float fs=float(s);\n"
    "        vec2 sp=vec2(sin(fs*3.7+t*0.5)*0.8,cos(fs*2.3+t*0.4)*0.6);\n"
    "        float sd=length(kuv-sp);\n"
    "        float twinkle=0.5+0.5*sin(fs*7.0+t*12.0);\n"
    "        float bval=spec(mod(fs*3.0,64.0)/64.0);\n"
    "        col+=hsv2rgb(vec3(mod(fs*0.02+t*0.05,1.0),0.4,1.0))*0.006/(sd+0.004)*(0.2+bval*0.5)*twinkle;\n"
    "    }\n"
    "    for(int s2=0;s2<50;s2++){float fs2=float(s2);\n"
    "        vec2 sp2a=vec2(sin(fs2*3.7+t*0.5)*0.8,cos(fs2*2.3+t*0.4)*0.6);\n"
    "        int next=(s2+1<50)?s2+1:0; float fn=float(next);\n"
    "        vec2 sp2b=vec2(sin(fn*3.7+t*0.5)*0.8,cos(fn*2.3+t*0.4)*0.6);\n"
    "        if(length(sp2a-sp2b)<0.5){\n"
    "            vec2 ab=sp2b-sp2a; float abl=length(ab); vec2 abd=ab/(abl+0.001);\n"
    "            float proj=clamp(dot(kuv-sp2a,abd),0.0,abl); vec2 cl=sp2a+abd*proj;\n"
    "            float d=length(kuv-cl);\n"
    "            col+=vec3(0.4,0.5,0.8)*0.001/(d+0.002)*0.15*(0.2+u_energy*0.2);\n"
    "        }\n"
    "    }\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Neon Asteroid Storm: asteroid field + electric storm + plasma */
...
static const char *frag_smoketunnel =
    "float fbm_st(vec2 p){float v=0.0,a=0.5; for(int i=0;i<5;i++){v+=a*noise(p);p=p*2.1+vec2(1.7,9.2);a*=0.5;} return v;}\n"
    "void main() {\n"
    "    vec2 uv=(gl_FragCoord.xy/u_resolution-0.5)*2.0*vec2(u_resolution.x/u_resolution.y,1.0);\n"
    "    float t=u_time*0.4; float dist=length(uv)+0.001; float ang=atan(uv.y,uv.x);\n"
    "    float tunnel_d=1.0/dist; float tunnel_a=ang/3.14159;\n"
    ...
    "    float thue=mod(0.6+tunnel_d*0.04+t*0.02,1.0);\n"
    "    vec3 col=hsv2rgb(vec3(thue,0.5,0.12+tunnel_v*0.25+u_energy*0.06));\n"
    "    vec2 tuv=vec2(tunnel_a,tunnel_d*0.5);\n"
    "    vec2 sp=tuv*3.0+vec2(1.5);\n"
    "    vec2 curl=vec2(fbm_st(sp+vec2(t*0.8,0)+u_bass*0.3),fbm_st(sp+vec2(0,t*0.7)+u_mid*0.3));\n"
    "    float fluid=fbm_st(sp+curl*1.5+vec2(t*0.5,-t*0.4));\n"
    "    float fhue=mod(fluid*0.3+curl.x*0.15+t*0.03,1.0);\n"
    "    col+=hsv2rgb(vec3(fhue,0.45,1.0))*fluid*0.2*exp(-dist*0.4)*(0.3+u_energy*0.3);\n"
    "    float smoke_detail=fbm_st(tuv*6.0+curl*2.0+vec2(t*0.6));\n"
    "    col+=hsv2rgb(vec3(mod(fhue+0.3,1.0),0.4,1.0))*smoke_detail*0.08*(0.3+u_energy*0.25);\n"
    "    float fire_a=noise(vec2(tunnel_a*8.0,tz*2.5))*0.4\n"
    "        +noise(vec2(tunnel_a*16.0,tz*5.0))*0.2;\n"
    "    float edge_fire=fire_a*exp(-dist*0.3)*(0.35+u_energy*0.4+u_beat*0.2);\n"
    "    float bf=clamp(edge_fire,0.0,1.0);\n"
    "    if(bf>0.05){\n"
    "        if(bf<0.3) col+=vec3(0.02,0.04,bf*3.0+0.1)*0.35;\n"
    "        else if(bf<0.6){float g=(bf-0.3)*3.33;col+=vec3(0.02,g*0.2,0.5+g*0.45)*0.35;}\n"
    "        else{float g=(bf-0.6)*2.5;col+=vec3(g*0.15,0.2+g*0.3,0.95)*0.35;}\n"
    "    }\n"
    "    float core=exp(-dist*dist*7.0)*(0.3+u_bass*0.6+u_beat*0.3);\n"
    "    col+=vec3(0.3,0.5,1.0)*core*0.45;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Smoke Nebula: smoke fluid + nebula clouds + starburst rays */
...
static const char *frag_auroramatrix =
    "void main() {\n"
    "    vec2 uv=(gl_FragCoord.xy/u_resolution-0.5)*2.0*vec2(u_resolution.x/u_resolution.y,1.0);\n"
    "    vec2 uv01=gl_FragCoord.xy/u_resolution; float t=u_time;\n"
    "    float dist=length(uv)+0.001; float ang=atan(uv.y,uv.x);\n"
    ...
    "    vec2 kuv=vec2(cos(ka),sin(ka))*dist;\n"
    "    vec2 kuv01=(kuv/vec2(u_resolution.x/u_resolution.y,1.0)+1.0)*0.5;\n"
    "    vec3 col=hsv2rgb(vec3(mod(0.55+kuv01.y*0.1+t*0.015,1.0),0.5,0.12+u_energy*0.06));\n"
    "    for(int layer=0;layer<5;layer++){float fl=float(layer);\n"
    "        float wave=sin(kuv01.x*8.0+t*0.5+u_bass*2.0+fl*1.4)*0.5\n"
    "            +sin(kuv01.x*12.0+t*1.0+fl*0.7)*0.3;\n"
    "        float center=0.15+fl*0.16+wave*0.04;\n"
    "        float band=exp(-(kuv01.y-center)*(kuv01.y-center)*50.0)*0.22;\n"
    "        float ahue=mod(0.35+fl*0.09+t*0.02,1.0);\n"
    "        col+=hsv2rgb(vec3(ahue,0.6,1.0))*band*(0.3+u_energy*0.3+u_beat*0.12);\n"
    "    }\n"
    "    float cols=22.0+u_bass*5.0;\n"
    "    float col_id=floor(kuv01.x*cols);\n"
    "    float col_hash=hash(vec2(col_id*0.773,col_id*1.31));\n"
    "    float speed=(0.3+col_hash*1.2)*(1.0+u_energy*1.8+u_beat*0.8);\n"
    "    float scroll=t*speed+col_hash*20.0;\n"
    "    float pos_in_trail=fract(kuv01.y-scroll);\n"
    "    float fade=pow(pos_in_trail,2.0+col_hash*2.0);\n"
    "    float tip=smoothstep(0.95,1.0,pos_in_trail);\n"
    "    float col_spec=spec(mod(col_id*1.5,64.0)/64.0);\n"
    "    float digit_bright=fade*(0.12+col_spec*0.2)*0.5;\n"
    "    col+=vec3(0.1,0.7,0.3)*digit_bright;\n"
    "    col+=vec3(0.7,1.0,0.8)*tip*0.12*col_spec;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Star Grid: constellation stars + maze grid + plasma aurora bands */
...
static const char *frag_polyhedranebula =
    "void main() {\n"
    "    vec2 uv=(gl_FragCoord.xy/u_resolution-0.5)*2.0*vec2(u_resolution.x/u_resolution.y,1.0);\n"
    "    float t=u_time*0.4; float dist=length(uv)+0.001; float ang=atan(uv.y,uv.x);\n"
    "    float twist=ang+t*2.0+sin(dist*4.0-t*3.5)*0.4*(1.0+u_bass*0.4);\n"
    "    vec2 wuv=vec2(cos(twist),sin(twist))*dist;\n"
    "    float n1=noise(wuv*2.5+vec2(t*0.15,t*0.1))*0.4;\n"
    "    float n2=noise(wuv*5.0+vec2(-t*0.08,t*0.18))*0.25;\n"
    "    float neb=n1+n2;\n"
    "    neb*=(0.35+u_energy*0.4+u_bass*0.15);\n"
    "    float nhue=mod(0.6+neb*0.3+dist*0.08+t*0.025,1.0);\n"
    "    vec3 col=hsv2rgb(vec3(nhue,0.55,0.12+neb*0.3+u_energy*0.06));\n"
    ...
    "    col+=hsv2rgb(vec3(mod(nhue+0.2,1.0),0.5,1.0))*vortex_v*0.15;\n"
    "    for(int i=0;i<8;i++){float fi=float(i);\n"
    "        float oa=fi*0.785+t*1.5*0.6*(0.5+hash(vec2(fi,1.0))*0.5);\n"
    "        float orr=0.3+sin(t*0.25+fi*1.7)*0.2;\n"
    "        vec2 center=vec2(cos(oa)*orr,sin(oa)*orr);\n"
    "        float rot=t*2.5+fi*1.1;\n"
    "        float sides=3.0+mod(fi,4.0);\n"
    "        vec2 lp=uv-center;\n"
    "        float la=atan(lp.y,lp.x)+rot;\n"
    "        float lr=length(lp);\n"
    ...
    "        float glow=0.004/(abs(poly_d)+0.004);\n"
    "        float sp=spec(mod(fi*8.0,64.0)/64.0);\n"
    "        float phue=mod(nhue+fi*0.1+t*0.04,1.0);\n"
    "        col+=hsv2rgb(vec3(phue,0.7,1.0))*glow*(0.15+sp*0.3+u_beat*0.15);\n"
    "        float fill=smoothstep(0.005,-0.01,poly_d)*0.15*(0.3+sp*0.3);\n"
    "        col+=hsv2rgb(vec3(phue,0.5,0.6))*fill;\n"
    "    }\n"
    "    float core=exp(-dist*dist*7.0)*(0.3+u_bass*0.5+u_beat*0.3);\n"
    "    col+=hsv2rgb(vec3(mod(t*0.06,1.0),0.35,1.0))*core*0.4;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Volcanic Vortex: lava + vortex spin + starburst eruption */
...
static const char *frag_matrixaurora =
    ...
    "    vec3 col=hsv2rgb(vec3(mod(0.55+kuv01.y*0.1+t*0.015,1.0),0.5,0.12+u_energy*0.06));\n"
    "    for(int layer=0;layer<5;layer++){float fl=float(layer);\n"
    "        float wave=sin(kuv01.x*7.0+t*0.45+u_bass*2.0+fl*1.3)*0.5;\n"
    "        float center=0.15+fl*0.16+wave*0.04;\n"
    "        float band=exp(-(kuv01.y-center)*(kuv01.y-center)*45.0)*0.22;\n"
    "        float ahue=mod(0.35+fl*0.1+t*0.02,1.0);\n"
    "        col+=hsv2rgb(vec3(ahue,0.6,1.0))*band*(0.3+u_energy*0.3+u_beat*0.15);\n"
    "    }\n"
    "    float cols=20.0+u_bass*5.0;\n"
    "    float col_id=floor(kuv01.x*cols);\n"
    "    float col_hash=hash(vec2(col_id*0.773,col_id*1.31));\n"
    ...
    "    float tip=smoothstep(0.95,1.0,pos_in_trail);\n"
    "    float col_spec=spec(mod(col_id*1.5,64.0)/64.0);\n"
    "    float digit_bright=fade*(0.15+col_spec*0.25)*0.45;\n"
    "    col+=vec3(0.1,0.7,0.3)*digit_bright;\n"
    "    col+=vec3(0.7,1.0,0.8)*tip*0.12*col_spec;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Shockwave Kaleidoscope: shockwave rings + radial kaleidoscope + fire fill */
...
static const char *frag_spectrumvortex =
    ...
    "    vec2 uv01=gl_FragCoord.xy/u_resolution; float t=u_time;\n"
    "    float dist=length(uv)+0.001; float ang=atan(uv.y,uv.x);\n"
    "    float twist=ang+t*2.5+sin(dist*4.0-t*3.5)*0.45*(1.0+u_bass*0.4);\n"
    "    float vortex_v=sin(twist*5.0)*0.5+0.5;\n"
    "    vortex_v*=exp(-dist*0.9)*(0.25+u_energy*0.35);\n"
    "    float vhue=mod(0.5+twist*0.04+t*0.02,1.0);\n"
    "    vec3 col=hsv2rgb(vec3(vhue,0.5,0.12+vortex_v*0.25+u_energy*0.06));\n"
    "    float warp_ang=twist;\n"
    "    float warp01=warp_ang/6.28318+0.5;\n"
    "    float N=48.0;\n"
    "    float seg=floor(warp01*N); float sc=(seg+0.5)/N;\n"
    "    float sv=spec(sc); float boost=1.0+u_beat*0.5;\n"
    "    float barH=sv*0.35*boost;\n"
    "    float barStart=0.15;\n"
    "    float barEnd=barStart+barH;\n"
    "    float in_bar=step(barStart,dist)*step(dist,barEnd);\n"
    "    float seg_gap=smoothstep(0.0,0.1,fract(warp01*N))*smoothstep(1.0,0.9,fract(warp01*N));\n"
    "    in_bar*=seg_gap;\n"
    "    float bhue=mod(sc*0.7+t*0.04,1.0);\n"
    "    float grad=(dist-barStart)/max(barH,0.01);\n"
    "    col+=hsv2rgb(vec3(bhue,0.8,0.3+grad*0.6))*in_bar*0.6;\n"
    "    float tip=exp(-abs(dist-barEnd)*50.0)*sv*0.4*seg_gap;\n"
    "    col+=hsv2rgb(vec3(bhue,0.3,1.0))*tip;\n"
    "    for(int ring=0;ring<6;ring++){float fr=float(ring);\n"
    "        float age=mod(t*0.7+fr*0.55,3.5);\n"
    "        float radius=age*0.25*(1.0+u_bass*0.12);\n"
    "        float ring_d=abs(dist-radius);\n"
    "        float fade=(1.0-age/3.5);\n"
    "        float ring_v=exp(-ring_d*ring_d*180.0)*fade*0.3;\n"
    "        float rhue=mod(0.5+fr*0.09+t*0.03,1.0);\n"
    "        col+=hsv2rgb(vec3(rhue,0.5,1.0))*ring_v*(0.35+u_energy*0.3+u_beat*0.2);\n"
    "    }\n"
    "    float core=exp(-dist*dist*9.0)*(0.3+u_bass*0.55+u_beat*0.3);\n"
    "    col+=hsv2rgb(vec3(mod(t*0.06,1.0),0.3,1.0))*core*0.4;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Bubble Kaleidoscope: rainbow bubbles + radial kaleidoscope + nebula */
...
static const char *frag_particlekaleidoscope =
    "void main() {\n"
    "    vec2 uv=(gl_FragCoord.xy/u_resolution-0.5)*2.0*vec2(u_resolution.x/u_resolution.y,1.0);\n"
    "    vec2 uv01=gl_FragCoord.xy/u_resolution; float t=u_time;\n"
    "    float dist=length(uv)+0.001; float ang=atan(uv.y,uv.x);\n"
    ...
    "    float fire_n=noise(vec2(kuv01.x*6.0,kuv01.y*7.0-t*2.2))*0.5\n"
    "        +noise(vec2(kuv01.x*12.0,kuv01.y*14.0-t*3.5))*0.25;\n"
    "    fire_n*=(0.3+u_energy*0.35+u_beat*0.12)*(1.0-kuv01.y*0.3);\n"
    "    float fv=clamp(fire_n,0.0,1.0);\n"
    "    vec3 col;\n"
    "    if(fv<0.25) col=vec3(fv*3.0+0.12,fv*0.4+0.03,0.02);\n"
    "    else if(fv<0.5){float g=(fv-0.25)*4.0;col=vec3(0.75+g*0.25,g*0.5+0.03,0.02);}\n"
    "    else{float g=(fv-0.5)*2.0;col=vec3(1.0,0.5+g*0.4,g*0.2);}\n"
    "    for(int i=0;i<160;i++){float fi=float(i);\n"
    "        float h1=hash(vec2(fi*0.73,fi*1.17)),h2=hash(vec2(fi*1.91,fi*0.43));\n"
    "        float h3=hash(vec2(fi*2.37,fi*0.67));\n"
    "        float px=(h1*2.0-1.0)*0.8;\n"
    "        float py=(h2*2.0-1.0)*0.6;\n"
    "        float drift_x=sin(t*0.8+fi*2.3)*0.05*(1.0+u_energy*0.5);\n"
    "        float drift_y=cos(t*0.7+fi*1.9)*0.05*(1.0+u_energy*0.5);\n"
    "        float pd=length(kuv-vec2(px+drift_x,py+drift_y));\n"
    "        float bval=spec(mod(fi*2.0,64.0)/64.0);\n"
    "        float sz=0.003+bval*0.004;\n"
    "        float pglow=sz/(pd+sz)*0.35*(0.12+bval*0.3+u_energy*0.12);\n"
    "        float phue=mod(0.08+fi*0.003+t*0.02,1.0);\n"
    "        col+=hsv2rgb(vec3(phue,0.55,1.0))*pglow*0.12;\n"
    "    }\n"
    "    float core=exp(-dist*dist*7.0)*(0.3+u_bass*0.5+u_beat*0.3);\n"
    "    col+=vec3(1.0,0.75,0.3)*core*0.35;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Asteroid Vortex: asteroid field + vortex spin + fire burn-up */
static const char *frag_asteroidvortex =
    "void main() {\n"
    "    vec2 uv=(gl_FragCoord.xy/u_resolution-0.5)*2.0*vec2(u_resolution.x/u_resolution.y,1.0);\n"
    "    vec2 uv01=gl_FragCoord.xy/u_resolution; float t=u_time;\n"
    "    float dist=length(uv)+0.001; float ang=atan(uv.y,uv.x);\n"
    "    float twist=ang+t*2.5+sin(dist*5.0-t*3.5)*0.5*(1.0+u_bass*0.45);\n"
    "    float vortex_v=sin(twist*4.0)*0.5+0.5;\n"
    "    vortex_v*=exp(-dist*0.8)*(0.25+u_energy*0.35);\n"
    "    float vhue=mod(0.08+twist*0.03+t*0.02,1.0);\n"
    "    vec3 col=hsv2rgb(vec3(vhue,0.55,0.12+vortex_v*0.25+u_energy*0.06));\n"
    "    float fire_n=noise(vec2(uv01.x*6.0,uv01.y*7.0-t*2.2))*0.5\n"
    "        +noise(vec2(uv01.x*12.0,uv01.y*14.0-t*3.5))*0.25;\n"
    "    fire_n*=(0.25+u_energy*0.3+u_beat*0.1)*exp(-dist*0.5);\n"
    "    float fv=clamp(fire_n,0.0,1.0);\n"
    "    if(fv>0.05){\n"
    "        if(fv<0.3) col+=vec3(fv*2.5+0.08,fv*0.3,0.02)*0.3;\n"
    "        else col+=vec3(0.9,fv*0.6,0.05)*0.3;\n"
    "    }\n"
    "    float fly_speed=0.4+u_energy*0.7+u_beat*0.5;\n"
    "    for(int s=0;s<45;s++){float fs=float(s);\n"
    "        float h1=hash(vec2(fs*0.73,fs*1.17)),h2=hash(vec2(fs*1.91,fs*0.43));\n"
    "        float h3=hash(vec2(fs*2.37,fs*0.67));\n"
    "        float sa=h1*6.283+t*0.5/(h2*0.5+0.5);\n"
    "        float sr=0.1+h2*0.8-t*0.03*(1.0+u_energy*0.5);\n"
    "        sr=max(mod(sr,0.9),0.05);\n"
    "        vec2 ap=vec2(cos(sa)*sr,sin(sa)*sr);\n"
    "        float ad=length(uv-ap);\n"
    "        float sz=0.008+h3*0.012;\n"
    "        float bval=spec(mod(fs*3.0,64.0)/64.0);\n"
    "        if(ad<sz*3.0){\n"
    "            float glow=sz/(ad+sz)*0.5*(0.2+bval*0.35+u_energy*0.2);\n"
    "            float heat=1.0-sr;\n"
    "            float ahue=mod(0.08+heat*0.08+fs*0.01,1.0);\n"
    "            col+=hsv2rgb(vec3(ahue,0.5+heat*0.3,1.0))*glow*0.25;\n"
    "        }\n"
    "        if(sr<0.25){\n"
    "            vec2 trail_dir=normalize(ap+0.001)*0.02;\n"
    "            for(int tt=1;tt<3;tt++){float ft=float(tt);\n"
    "                vec2 tp=ap+trail_dir*ft;\n"
    "                float td=length(uv-tp);\n"
    "                col+=vec3(1.0,0.5,0.1)*0.003/(td+0.003)*(0.1+u_energy*0.1)/(ft*0.8);\n"
    "            }\n"
    "        }\n"
    "    }\n"
    "    float core=exp(-dist*dist*8.0)*(0.35+u_bass*0.6+u_beat*0.35);\n"
    "    col+=vec3(1.0,0.7,0.2)*core*0.45;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";

/* Asteroid Kaleidoscope: asteroid field + kaleidoscope mirror + fire trails */
