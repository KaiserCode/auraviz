...
static const char *frag_aurora =
    ...
    "    float sky_h=mod(0.6+uv.y*0.08+t*0.01,1.0);\n"
    "    col=hsv2rgb(vec3(sky_h,0.4,0.08+uv.y*0.06+u_energy*0.04));\n"
    "    for(int layer=0;layer<6;layer++){float fl=float(layer);\n"
    "        float wave=sin(uv.x*5.0+t*(0.8+fl*0.25)+u_bass*2.5+fl*1.3)*0.5\n"
    "            +sin(uv.x*12.0+t*1.2+fl*0.7+u_mid*2.0)*0.3\n"
    "            +sin(uv.x*3.0+t*0.5+fl*2.0)*0.2;\n"
    "        float center=0.3+fl*0.1+wave*0.1;\n"
    "        float spread=40.0+fl*10.0-u_bass*8.0;\n"
    "        float band=exp(-(uv.y-center)*(uv.y-center)*spread);\n"
    "        band+=exp(-(uv.y-center-0.15)*(uv.y-center-0.15)*spread*0.5)*0.3;\n"
    "        band+=exp(-(uv.y-center+0.1)*(uv.y-center+0.1)*spread*0.7)*0.2;\n"
    "        float hue=mod(0.25+fl*0.08+uv.x*0.06+wave*0.05+t*0.02,1.0);\n"
    "        float bright=band*(0.5+u_energy*0.5+u_beat*0.25);\n"
    "        col+=hsv2rgb(vec3(hue,0.75,1.0))*bright;\n"
    "    }\n"
    "    float shimmer=noise(vec2(uv.x*20.0,uv.y*10.0+t*2.0))*0.08*(0.5+u_treble);\n"
    "    col+=vec3(0.1,0.2,0.15)*shimmer;\n"
    "    col+=vec3(0.05,0.03,0.08);\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";
...
static const char *frag_dna =
    "void main() {\n"
    "    vec2 uv=(gl_FragCoord.xy/u_resolution-0.5)*2.0*vec2(u_resolution.x/u_resolution.y,1.0);\n"
    "    float t=u_time; vec3 col=vec3(0.03,0.02,0.05);\n"
    "    float speed=1.5+u_energy*2.0+u_beat;\n"
    "    for(int strand=0;strand<4;strand++){\n"
    "        float fs=float(strand);\n"
    "        float orbit_a=t*0.2+fs*1.5708;\n"
    "        float orbit_r=0.45+sin(t*0.12+fs*2.0)*0.25;\n"
    "        float cx=cos(orbit_a)*orbit_r;\n"
    "        float cy=sin(orbit_a)*orbit_r;\n"
    "        float rot=t*0.4+fs*0.5;\n"
    ...
    "            float scroll=rl.y*6.0+t*speed+fs*2.0;\n"
    ...
    "        }\n"
    "    }\n"
    "    col+=vec3(0.04,0.03,0.06)*(1.0+noise(uv*3.0+vec2(t*0.1,0.0))*0.3);\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";
...
static const char *frag_rainbowbubbles =
    ...
    "        float rise_speed=0.15+bh3*0.3+u_energy*0.2;\n"
    "        float bx=(bh2*2.0-1.0)*ar*0.8+sin(t*0.6+fi*2.7)*0.15*(1.0+u_bass*0.5);\n"
    ...
    "            float hue=fract(film_thick*0.3+fi*0.07+t*0.12+u_bass*0.2+u_energy*0.15);\n"
    ...
    "            bubble+=hsv2rgb(vec3(mod(hue+0.5,1.0),0.8,1.0))*u_beat*0.25*rim;\n"
    ...
    "        float gh=fract(fi*0.17+t*0.05+u_energy*0.1);\n"
...
static const char *frag_3dmaze =
    ...
    "    uv.x*=u_resolution.x/u_resolution.y;\n"
    "    float t=u_time;\n"
    "    float spd=0.6+u_energy*0.4+u_beat*0.2;\n"
    "    float path_t=t*spd*0.3;\n"
    "    float seg=floor(path_t);\n"
    "    float frac_t=fract(path_t);\n"
    "    float smooth_t=frac_t*frac_t*(3.0-2.0*frac_t);\n"
    "    float ang1=hash(vec2(seg*0.73,seg*1.31))*6.283;\n"
    "    float ang2=hash(vec2((seg+1.0)*0.73,(seg+1.0)*1.31))*6.283;\n"
    "    float ang_diff=ang2-ang1;\n"
    "    ang_diff=ang_diff-floor(ang_diff/6.283+0.5)*6.283;\n"
    "    float ang=ang1+ang_diff*smooth_t;\n"
    "    vec2 pos1=vec2(hash(vec2(seg,1.0)),hash(vec2(1.0,seg)))*8.0;\n"
    "    vec2 pos2=vec2(hash(vec2(seg+1.0,1.0)),hash(vec2(1.0,seg+1.0)))*8.0;\n"
    "    vec2 cam_pos=mix(pos1,pos2,smooth_t);\n"
    "    vec3 ro=vec3(cam_pos.x,0.0,cam_pos.y);\n"
    ...
    "            float wx=abs(fc.x-0.5)-0.4;\n"
    "            float wz=abs(fc.y-0.5)-0.4;\n"
    "            wall_d=min(wall_d,max(-wx,-wz));\n"
    "        }\n"
    "        if(maze_wall(cell+vec2(1.0,0.0))>0.5) wall_d=min(wall_d,abs(fc.x-0.96));\n"
    "        if(maze_wall(cell+vec2(-1.0,0.0))>0.5) wall_d=min(wall_d,abs(fc.x-0.04));\n"
    "        if(maze_wall(cell+vec2(0.0,1.0))>0.5) wall_d=min(wall_d,abs(fc.y-0.96));\n"
    "        if(maze_wall(cell+vec2(0.0,-1.0))>0.5) wall_d=min(wall_d,abs(fc.y-0.04));\n"
    "        float d=wall_d*0.4;\n"
    "        d=max(d,0.004);\n"
    "        total_d+=d;\n"
    ...
    "                if(gx>gz) hit_n=vec3(sign(fc.x-0.5),0.0,0.0);\n"
    "                else hit_n=vec3(0.0,0.0,sign(fc.y-0.5));}\n"
    "            break;}\n"
    "        if(total_d>25.0) break;\n"
    "    }\n"
    "    if(hit){\n"
    "        float fog=exp(-total_d*0.1);\n"
    "        float light=0.4+0.6*max(0.0,dot(hit_n,normalize(vec3(0.4,0.8,0.3))));\n"
    "        float sp=spec(mod(total_d*2.0,64.0)/64.0);\n"
    "        float torch=1.0/(1.0+total_d*total_d*0.03);\n"
    "        if(abs(hit_n.y)>0.5){\n"
    ...
    "            if(hit_n.y>0.0){\n"
    "                vec3 floor_col=mix(vec3(0.55,0.4,0.25),vec3(0.45,0.35,0.2),checker);\n"
    "                floor_col+=vec3(0.1,0.08,0.0)*hash(floor(vec2(hit_p.x,hit_p.z)*4.0));\n"
    "                col=floor_col*light*fog*(0.6+torch*0.4);\n"
    "            } else {\n"
    "                vec3 ceil_col=mix(vec3(0.45,0.45,0.55),vec3(0.35,0.35,0.45),checker*0.5);\n"
    "                ceil_col+=vec3(0.05,0.05,0.08)*hash(floor(vec2(hit_p.x,hit_p.z)*3.0));\n"
    "                col=ceil_col*light*fog*(0.5+torch*0.4);\n"
    "            }\n"
    "        } else {\n"
    ...
    "            float wall_var=hash(floor(vec2(hit_p.x+hit_p.z,hit_p.y)*4.0));\n"
    "            vec3 brick_col=mix(vec3(0.65,0.28,0.18),vec3(0.7,0.35,0.2),wall_var*0.5);\n"
    "            brick_col=mix(brick_col,vec3(0.55,0.5,0.4),bline*0.7);\n"
    "            brick_col+=vec3(0.08,0.04,0.02)*sp;\n"
    "            col=brick_col*light*fog*(0.6+torch*0.5);\n"
    "        }\n"
    "        col+=vec3(0.4,0.25,0.15)*u_bass*0.15*fog*torch;\n"
    "        col+=vec3(0.3,0.2,0.1)*u_beat*0.12*fog;\n"
    "    } else {\n"
...
static const char *frag_flyingwin98 =
    ...
    "            float hue_base=mod(fi*0.13+t*0.04,1.0);\n"
    "            float q_x=step(0.5,luv.x);\n"
    "            float q_y=step(0.5,luv.y);\n"
    "            float quad=q_x+q_y*2.0;\n"
    "            vec3 pane=vec3(0.0);\n"
    "            if(quad<0.5) pane=vec3(1.0,0.0,0.0);\n"
    "            else if(quad<1.5) pane=vec3(0.0,0.8,0.0);\n"
    "            else if(quad<2.5) pane=vec3(0.0,0.4,1.0);\n"
    ...
    "            win_col+=vec3(0.15,0.1,0.1)*u_bass*0.5;\n"
    "            col=mix(col,win_col*fade,0.9);\n"
    "        }\n"
    "        float glow_d=max(abs(diff.x)-sz,abs(diff.y)-sz*0.8);\n"
    "        col+=vec3(0.3,0.3,0.4)*0.002/(abs(glow_d)+0.002)*fade*0.3;\n"
    "    }\n"
    "    for(int s=0;s<30;s++){\n"
    "        float fs=float(s);\n"
    "        float sz2=fract(hash(vec2(fs*3.3,fs*1.7))+t*fly_speed*0.2);\n"
    "        vec2 sp2=vec2(hash(vec2(fs,5.0))-0.5,hash(vec2(5.0,fs))-0.5)*sz2*6.0;\n"
    "        float sd=length(uv-sp2);\n"
    "        float sf=smoothstep(0.0,0.1,sz2)*smoothstep(1.0,0.8,sz2);\n"
    "        col+=vec3(0.6,0.65,0.8)*0.001/(sd+0.001)*sf;\n"
    "    }\n"
    "    col+=vec3(0.7,0.7,0.9)*u_beat*0.03;\n"
    "    gl_FragColor = vec4(clamp(col,0.0,1.0), 1.0);\n}\n";
...
static const char *frag_diamondripple =
    ...
    "    float t=u_time;\n"
    "    float dist=length(uv);\n"
    "    float ripple=(sin(dist*18.0-t*4.0+u_bass*6.0)*0.5+0.5)*(sin(dist*10.0-t*2.5+u_mid*4.0)*0.3+0.7);\n"
    "    ripple*=(0.5+u_energy+u_beat*0.3)/(dist*1.5+0.4);\n"
    "    float rip_hue=mod(dist*0.2+t*0.05,1.0);\n"
    "    vec3 col=hsv2rgb(vec3(rip_hue,0.6,clamp(ripple*0.6+0.08,0.0,1.0)));\n"
    ...
    "        float tail_x=abs(uv01.x-cx);\n"
    "        float tail_dy=uv01.y-cy;\n"
    "        if(tail_x<sz*0.35 && tail_dy>0.0 && tail_dy<tail_len){\n"
    "            float tf=1.0-tail_dy/tail_len;\n"
    "            float tw=1.0-tail_x/(sz*0.35);\n"
