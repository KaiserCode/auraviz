 ...
 *
 * Fixed: AGC decay, band smoothing attack/release, DC removal,
 *        render multipliers, peak decay rates.
 *****************************************************************************/
...
/* ── Ring buffer for consistent analysis window ── */
...
typedef struct
{
    ...
    int   ring_pos;          /* next write position */
    ...
    /* AGC: short-window envelope instead of all-time peak */
    float agc_envelope;      /* smoothed RMS-like envelope */
    float agc_peak;          /* fast-attack, medium-release peak */
                          ...
                          int nb_samples, int channels)
{
    ...
    /* ── 2. Extract the most recent 1024 samples from ring ── */
    int N = 1024;
    float mono[1024];
    for (int i = 0; i < N; i++) {
        int idx = (p->ring_pos - N + i + RING_SIZE) % RING_SIZE;
        mono[i] = p->ring[idx];
    }
    ...
    /* ── 4. Compute RMS of this window for AGC envelope ── */
    float rms = 0;
    for (int i = 0; i < N; i++) rms += mono[i] * mono[i];
    rms = sqrtf(rms / N);
    ...
    /* ── 6. DFT: one bin per band at log-spaced center frequency ── */
    float raw_mag[NUM_BANDS];
    float frame_max = 0.0001f;
    for (int band = 0; band < NUM_BANDS; band++) {
        /* Log-spaced from 30 Hz to ~15 kHz */
        float freq = 30.0f * powf(500.0f, (float)band / (NUM_BANDS - 1));
        int k = (int)(freq * N / p->i_rate + 0.5f);
        if (k < 1) k = 1;
        if (k >= N/2) k = N/2 - 1;
    ...
    /* ── 7. AGC: short-window envelope tracking ──
     *
     * Instead of an all-time peak that barely decays, we track two things:
     *   - agc_envelope: smoothed RMS (follows the overall loudness)
     *   - agc_peak: fast-attack / medium-release peak
     *
     * The normalization reference is a blend of both, so it adapts quickly
     * when music gets quiet and doesn't get stuck high.
     */
    float gain_pct = p->gain / 100.0f;

    /* Envelope: EMA of RMS with user-controlled speed */
    float env_speed = 0.05f + gain_pct * 0.15f;  /* 0.05 (slow) to 0.20 (fast) */
    p->agc_envelope += (rms - p->agc_envelope) * env_speed;
    if (p->agc_envelope < 0.0001f) p->agc_envelope = 0.0001f;

    /* Peak: fast attack, medium release (much faster than the old 0.999) */
    float peak_release = 0.92f - gain_pct * 0.12f;  /* 0.92 (low gain) to 0.80 (high gain) */
    if (frame_max > p->agc_peak)
        p->agc_peak = frame_max;
    else
        p->agc_peak = p->agc_peak * peak_release + frame_max * (1.0f - peak_release);
    if (p->agc_peak < 0.0001f) p->agc_peak = 0.0001f;

    /* Normalization reference: blend of envelope and peak */
    float agc_ref = p->agc_envelope * 0.4f + p->agc_peak * 0.6f;
    if (agc_ref < 0.0001f) agc_ref = 0.0001f;

    /* Gain boost: user gain maps to a multiplier (0.5x to 3x) */
    float gain_mult = 0.5f + gain_pct * 2.5f;

    /* ── 8. Smoothing: proper attack/release ──
     *
     * Attack = how fast bars rise (should be fast for beat response)
     * Release = how fast bars fall (should be moderate, not glacial)
     */
    float smooth_pct = p->smooth / 100.0f;
    float attack  = 0.5f + (1.0f - smooth_pct) * 0.45f;  /* 0.5 (smooth) to 0.95 (sharp) */
    float release = 0.12f + (1.0f - smooth_pct) * 0.38f;  /* 0.12 (smooth) to 0.50 (sharp) */

    /* ── 9. Normalize and smooth bands ── */
    for (int band = 0; band < NUM_BANDS; band++) {
        float norm = (raw_mag[band] / agc_ref) * gain_mult;
        /* Perceptual compression: sqrt makes quiet details visible */
        float val = sqrtf(norm);
        if (val > 1.5f) val = 1.5f;  /* allow slight overshoot before clamp */
        ...
        if (p->smooth_bands[band] > 1.0f) p->smooth_bands[band] = 1.0f;
        if (p->smooth_bands[band] < 0.0f) p->smooth_bands[band] = 0.0f;

        p->bands[band] = val > 1.0f ? 1.0f : val;

        /* Peak dots: faster decay than before (0.90 instead of 0.96) */
        if (p->smooth_bands[band] > p->peak_bands[band])
            p->peak_bands[band] = p->smooth_bands[band];
        else
            p->peak_bands[band] *= (0.88f + smooth_pct * 0.06f);  /* 0.88 to 0.94 */
    }

    /* ── 10. Bass / Mid / Treble from smoothed bands ── */
    ...
    /* Bass/mid/treble smoothing */
    float bmt_attack  = 0.3f + (1.0f - smooth_pct) * 0.5f;
    float bmt_release = 0.1f + (1.0f - smooth_pct) * 0.3f;

    if (bass > p->bass)
        p->bass += (bass - p->bass) * bmt_attack;
    else
        p->bass += (bass - p->bass) * bmt_release;

    if (mid > p->mid)
        p->mid += (mid - p->mid) * bmt_attack;
    else
        p->mid += (mid - p->mid) * bmt_release;

    if (treble > p->treble)
        p->treble += (treble - p->treble) * bmt_attack;
    else
        p->treble += (treble - p->treble) * bmt_release;

    p->energy = (p->bass + p->mid + p->treble) / 3.0f;
}

/* ── Upscale half-res buffer to full ── */
 ...
 *  PRESETS
 *
 *  Key change: removed the *5 and *6 multipliers on smooth_bands.
 *  With correct AGC, smooth_bands already ranges [0..1] properly.
 *  Using *1.5 at most for a slight visual boost.
 * ====================================================================== */
...
static void render_spectrum(auraviz_thread_t *p, uint8_t *buf, int pitch)
{
    ...
    /* Dark background tinted by audio */
    for (int y = 0; y < h; y++) {
    ...
    for (int band = 0; band < NUM_BANDS; band++) {
        /* No more *6 — smooth_bands is already [0..1] with proper AGC */
        float val = p->smooth_bands[band] * 1.2f;
        if (val > 1) val = 1;
        float peak = p->peak_bands[band] * 1.2f;
        if (peak > 1) peak = 1;
        ...
        for (int y = my - bh; y < my; y++) {
            ...
            uint8_t r, g, b;
            hsv_fast(hue, 0.85f, 0.4f + 0.6f * (1 - pct), &r, &g, &b);
            uint8_t *row = buf + y * pitch;
                        ...
                        const float *samples, int nb_samples, int channels)
{
    ...
    for (int x = 0; x < w; x++) {
        ...
        while (hue >= 360) hue -= 360;
        uint8_t r, g, b;
        hsv_fast(hue, 0.9f, 0.7f + 0.3f * p->energy, &r, &g, &b);

        for (int dy = y0; dy <= y1 && dy < h; dy++) {
            put_pixel(buf + dy * pitch + x * 4, r, g, b);
            if (x > 0) {
...
static void render_circular(auraviz_thread_t *p, uint8_t *buf, int pitch)
{
    ...
    for (int band = 0; band < NUM_BANDS; band++) {
        ...
        /* Was *5, now *1.5 */
        float val = p->smooth_bands[band] * 1.5f;
        if (val > 1) val = 1;
...
static void render_kaleidoscope_half(auraviz_thread_t *p, uint8_t *hb, int hw, int hh)
{
    ...
    for (int py = 0; py < hh; py++) {
        ...
        for (int px = 0; px < hw; px++) {
            ...
            float bri = val * (0.5f + p->energy * 0.5f);
            if (bri > 1) bri = 1;
            uint8_t r, g, b;
...
static void render_starburst_half(auraviz_thread_t *p, uint8_t *hb, int hw, int hh)
{
    ...
    for (int py = 0; py < hh; py++) {
        ...
        for (int px = 0; px < hw; px++) {
            ...
            /* Was *6, now *1.5 */
            float rv = p->smooth_bands[ri] * 1.5f;
            if (rv > 1) rv = 1;
    ...
    for (int py = 0; py < hh; py++) {
        ...
        for (int px = 0; px < hw; px++) {
            ...
            for (int arm = 0; arm < 8; arm++) {
                ...
                /* Was *6, now *1.5 */
                bolt += arm_val * p->smooth_bands[arm * 8 % NUM_BANDS] * 1.5f;
            }
            if (bolt > 1) bolt = 1;
    ...
    for (int py = 0; py < hh; py++) {
        ...
        for (int px = 0; px < hw; px++) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                /* Was *30 on smooth_bands, now *8 */
                float freq = 20 + i * 8 + p->smooth_bands[i * 16 % NUM_BANDS] * 8;
                wave += sinf(d * freq - t * 4 - i * 1.5f) * (1.0f / (d * 8 + 1));
            }
            wave = wave * 0.25f + 0.5f;
            float hue = fmodf(wave * 180 + t * 20, 360);
            float val = 0.2f + wave * 0.6f + p->bass * 0.2f;
            if (val > 1) val = 1;
            if (val < 0) val = 0;
            uint8_t r, g, b;
...
static void render_glitch_half(auraviz_thread_t *p, uint8_t *hb, int hw, int hh)
{
    ...
    for (int py = 0; py < hh; py++) {
        ...
        for (int px = 0; px < hw; px++) {
            ...
            /* Was *5, now *1.5 */
            float bval = p->smooth_bands[band] * 1.5f;
            if (bval > 1) bval = 1;
    ...
    for (int py = 0; py < hh; py++) {
        ...
        for (int px = 0; px < hw; px++) {
            ...
            for (int layer = 0; layer < 3; layer++) {
                ...
                /* Was *3 on smooth_bands, now *1.5 */
                curtain += expf(-dist * dist * 80)
                         * (0.5f + p->smooth_bands[layer * 20 % NUM_BANDS] * 1.5f);
            }
...
static void render_fire_half(auraviz_thread_t *p, uint8_t *hb, int hw, int hh)
{
    ...
    for (int py = 0; py < hh; py++) {
        ...
        for (int px = 0; px < hw; px++) {
            ...
            float flame = (n1 + n2 + n3) * (1.0f - y) * (1.0f - y) * 1.5f
                        + p->bass * (1.0f - y) * 0.3f;
            if (flame > 1) flame = 1;
            if (flame < 0) flame = 0;
            uint8_t r, g, b;
        ...
        for (int px = 0; px < hw; px++) {
            ...
            for (int col = 0; col < 30; col++) {
                ...
                float speed = 1.5f + (col % 7) * 0.4f
                            + p->smooth_bands[col * 2 % NUM_BANDS] * 1.5f;
